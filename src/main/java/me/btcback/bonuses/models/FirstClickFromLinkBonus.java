...
public class FirstClickFromLinkBonus extends Bonus {

    public final String nameBonus = "from_link_bonus";
    final public Double sum = (Setts.getInstance().getDbl(this.nameBonus + "_sum") * 0.00000001);
    private volatile BotUser botUser;
    final private String subid2 = "link";
    ArrayList<Long> clickedOrdersAdId;

    public FirstClickFromLinkBonus(BotUser botUser) {
        this.botUser = botUser;
    }

    @Override
    public Boolean checkCanGetBonus() {
        clickedOrdersAdId = botUser.getClickedAdIdsInShortner(subid2);//botUser.getClickCountInAdmitad(subid2);
        if (clickedOrdersAdId != null && botUser.getJumpsFromLink() == 0 & clickedOrdersAdId.size() > 0) {
            return true;
        }
        return false;
    }
    ...
    public void fixUserBonus() {
        botUser.setJumpsFromLink(this.clickedOrdersAdId.size());
        botUser.saveMyJamps();
    }
